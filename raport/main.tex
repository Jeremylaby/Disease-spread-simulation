...
The simulation is implemented in Python using the Mesa framework for
agent-based modeling. Mesa provides the basic abstractions for agents,
models, and spatial environments. The population is placed on a
...
configuration explicit and easy to modify, pandas to store collected
simulation results, and Matplotlib to visualize both the final epidemic
curve and the live spatial state of the simulation. The most important
configurable parameters are the population size, grid dimensions,
...
\texttt{run\_live.py} using Matplotlib's interactive plotting mode. Each
frame renders the current grid state: susceptible, exposed, infected,
recovered, and deceased citizens are represented with different colors.
